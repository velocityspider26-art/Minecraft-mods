...
public class SeekerSpearMissileProjectileRenderer extends EntityRenderer<SeekerSpearMissileProjectileEntity> {
   private static final ResourceLocation texture = ResourceLocation.parse("crusty_chunks:textures/entities/seekerspear.png");
   private final ModelFireSpearRocket_Converted model;

   public SeekerSpearMissileProjectileRenderer(Context context) {
      super(context);
      this.model = new ModelFireSpearRocket_Converted(context.bakeLayer(ModelFireSpearRocket_Converted.LAYER_LOCATION));
   }

   public void render(
      SeekerSpearMissileProjectileEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn
   ) {
   ...
   public ResourceLocation getTextureLocation(SeekerSpearMissileProjectileEntity entity) {
      return texture;
   }
}
