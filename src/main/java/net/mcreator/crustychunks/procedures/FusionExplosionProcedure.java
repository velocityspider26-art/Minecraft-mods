...
public class FusionExplosionProcedure {
   public static void execute(LevelAccessor world, double x, double y, double z) {
      ...
      if (space) {
         CrustyChunksMod.queueServerWork(1, () -> SpaceFusionExplosionProcedure.execute(world, x, y, z));
      } else {
         ...
         if (world instanceof ServerLevel projectileLevel) {
            Projectile _entityToSpawn = (new Object() {
                  public Projectile getArrow(Level level, float damage, int knockback) {
                     AbstractArrow entityToSpawn = new FusionBlastEntityEntity((EntityType<? extends FusionBlastEntityEntity>)CrustyChunksModEntities.FUSION_BLAST_ENTITY.get(), level) {
               @Override
               protected void doKnockback(LivingEntity livingEntity, DamageSource damageSource) {
                  if (knockback > 0) {
                     double _kbres = Math.max(0.0, 1.0 - livingEntity.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE));
         ...
         if (world instanceof ServerLevel projectileLevel) {
            Projectile _entityToSpawn = (new Object() {
                  public Projectile getArrow(Level level, float damage, int knockback) {
                     AbstractArrow entityToSpawn = new FusionEffectProjectileEntity((EntityType<? extends FusionEffectProjectileEntity>)CrustyChunksModEntities.FUSION_EFFECT_PROJECTILE.get(), level) {
               @Override
               protected void doKnockback(LivingEntity livingEntity, DamageSource damageSource) {
                  if (knockback > 0) {
                     double _kbres = Math.max(0.0, 1.0 - livingEntity.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE));
         ...
         net.mcreator.crustychunks.compat.WariumSafety.report("FusionExplosionProcedure.execute", _wtSafe);
      }
   }
}
