...
public class ArtillerySolidProjectileRenderer extends EntityRenderer<ArtillerySolidProjectileEntity> {
   private static final ResourceLocation texture = ResourceLocation.parse("crusty_chunks:textures/entities/solidshell.png");
   private final Modelartilleryshell model;

   public ArtillerySolidProjectileRenderer(Context context) {
      super(context);
      this.model = new Modelartilleryshell(context.bakeLayer(Modelartilleryshell.LAYER_LOCATION));
   }

   public void render(
      ArtillerySolidProjectileEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn
   ) {
   ...
   public ResourceLocation getTextureLocation(ArtillerySolidProjectileEntity entity) {
      return texture;
   }
}
