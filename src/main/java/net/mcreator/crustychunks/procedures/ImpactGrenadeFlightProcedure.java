...
public class ImpactGrenadeFlightProcedure {
   public static void execute(LevelAccessor world, double x, double y, double z, Entity immediatesourceentity) {
      try {
      if (immediatesourceentity != null) {
         double RocketVelocity = 0.0;
         if (immediatesourceentity.isUnderWater()) {
            ImpactGrenadeHitProcedure.execute(world, immediatesourceentity);
         }
         ...
         net.mcreator.crustychunks.compat.WariumSafety.report("ImpactGrenadeFlightProcedure.execute", _wtSafe);
      }
   }
}
