...
public class BlackholeRenderer implements CelestialRenderer {
    ...
    public void invoke(@NotNull RenderLevelStageEvent event, @NotNull Celestial toRender, @NotNull VantagePoint vantagePoint) {
        ...
        Vector3dc observerPosition = CelestialRenderCoordinates.getObserverCelestialPosition(vantagePoint, event.getCamera().getPosition());

        float opacity = 1f;
        if (vantagePoint instanceof VantagePoint.OnCelestial) {
            double distance = observerPosition.distance(position);
        ...
        renderBlackhole(event.getPoseStack(), blackholeBuffer, toRender.getActualSize(), position, rotation, opacity);
        bufferSource.endBatch(getBlackholeRenderType());
    }

    private void renderBlackhole(PoseStack poseStack, VertexConsumer buffer, double size, Vector3dc center, Quaterniondc localRotation, float opacity) {
        ...
        matrix.translate((float) center.x(), (float) center.y(), (float) center.z());
