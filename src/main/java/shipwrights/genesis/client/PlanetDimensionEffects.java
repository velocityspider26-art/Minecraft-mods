...
public class PlanetDimensionEffects extends DimensionSpecialEffects {
    ...
    /** 0 on the ground, 1 at the edge of space. Drives the transition band. */
    private static double cachedAltitudeFactor = 0.0;

    /** The body's intrinsic atmosphere density, before altitude thins it. */
    private static double cachedBodyDensity = 1.0;

    /** The colour this body's sky passes through on its way to black. */
    private static float[] cachedTransitionRGB = {0.28f, 0.16f, 0.45f};

    /**
     * The tint the sky passes through between ground colour and space.
     *
     * <p>Taken from the body's data when it says, so Venus can go orange and
     * Mars dark red rather than everything going the same violet. When it does
     * not say, a violet is derived from the body's own sky colour — dimmed and
     * pushed toward blue, which is the direction a real sky shifts as the air
     * thins and only the shortest wavelengths are still being scattered.</p>
     */
    private static float[] transitionRGB(PlanetProperties props) {
        if (props == null) {
            return new float[]{0.28f, 0.16f, 0.45f};
        }
        Optional<PlanetColorPalette> declared = props.atmosphere().transitionColor();
        if (declared.isPresent() && !declared.get().isOverworld()) {
            int[] rgb = declared.get().getRGB();
            return new float[]{rgb[0] / 255.0f, rgb[1] / 255.0f, rgb[2] / 255.0f};
        }

        PlanetColorPalette base = props.atmosphere().color();
        if (base.isOverworld()) {
            // Earth's ground sky is sampled per-biome, so there is no single
            // colour to derive from. Use the violet its real sky turns.
            return new float[]{0.28f, 0.16f, 0.45f};
        }
        int[] rgb = base.getRGB();
        return new float[]{
                (rgb[0] / 255.0f) * 0.55f,
                (rgb[1] / 255.0f) * 0.35f,
                Math.min(1.0f, (rgb[2] / 255.0f) * 0.85f + 0.18f)};
    }
    ...
    public boolean renderSky(ClientLevel level, int unused, float partialTick, Matrix4f modelViewMatrix, Camera camera, Matrix4f projectionMatrix, boolean isFoggy, Runnable setupFog) {
        ...
        // 0 at the ground, 1 at the edge of space. The sky's colour depends on
        // how much air is still above you, which is what this measures.
        cachedAltitudeFactor = 1.0 - densityFade;
        // The body's own atmosphere, before altitude thins it. The band needs
        // this rather than the faded value, which is zero exactly where the
        // band would be strongest.
        cachedBodyDensity = density;
        cachedTransitionRGB = transitionRGB(planetProps);
        density *= densityFade;
    ...
    public static Vec3 getSkyColor(Vec3 position, float partialTick, long time, ClientLevel level, PlanetColorPalette palette) {
        ...
        // The violet band. Strongest halfway out of the atmosphere and gone at
        // both ends, so the ground sky and space itself are left alone. Scaled
        // by how much air the body actually has: a thin atmosphere shows a hint
        // of it and an airless one shows none, which is why the Moon goes
        // straight to black without needing a special case.
        float band = (float) (Math.sin(Math.PI * cachedAltitudeFactor) * cachedBodyDensity * 0.75);
        if (band > 0.0f) {
            r = Mth.lerp(band, r, cachedTransitionRGB[0] * intensity);
            g = Mth.lerp(band, g, cachedTransitionRGB[1] * intensity);
            b = Mth.lerp(band, b, cachedTransitionRGB[2] * intensity);
        }
