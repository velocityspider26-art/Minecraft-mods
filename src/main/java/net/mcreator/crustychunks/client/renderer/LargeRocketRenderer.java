...
public class LargeRocketRenderer extends EntityRenderer<LargeRocketEntity> {
   private static final ResourceLocation texture = ResourceLocation.parse("crusty_chunks:textures/entities/largerocket.png");
   private final ModelLargeRocket model;

   public LargeRocketRenderer(Context context) {
      super(context);
      this.model = new ModelLargeRocket(context.bakeLayer(ModelLargeRocket.LAYER_LOCATION));
   }

   public void render(LargeRocketEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
   ...
   public ResourceLocation getTextureLocation(LargeRocketEntity entity) {
      return texture;
   }
}
