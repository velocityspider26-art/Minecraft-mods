...
public class SemiAutomaticPistolEntitySwingsItemProcedure {
   public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
      ...
      if (entity != null) {
            ...
            if (!itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getBoolean("action")) {
               ...
               if (1.0 <= itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("Ammo")) {
                  if (itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getBoolean("Loaded")) {
                     if (entity.getData(CrustyChunksModVariables.PLAYER_VARIABLES)
                        .AimDownSights) {
                        if (itemstack.getItem() instanceof SemiAutomaticPistolAnimatedItem) {
                           CustomData.update(DataComponents.CUSTOM_DATA, itemstack, _tagupd -> _tagupd.putString("geckoAnim", "sightfire"));
                        }
                     } else if (itemstack.getItem() instanceof SemiAutomaticPistolAnimatedItem) {
         ...
         net.mcreator.crustychunks.compat.WariumSafety.report("SemiAutomaticPistolEntitySwingsItemProcedure.execute", _wtSafe);
      }
   }
}
