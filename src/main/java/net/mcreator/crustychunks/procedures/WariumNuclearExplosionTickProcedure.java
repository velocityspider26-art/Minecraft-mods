...
public class WariumNuclearExplosionTickProcedure {
   public static void execute(LevelAccessor world, double x, double y, double z, double power, double time) {
      if (!ModList.get().isLoaded("explosionoverhaul") && world instanceof Level level && !level.isClientSide()) {
         double _waveScale;
         try {
            _waveScale = net.mcreator.crustychunks.WariumConfig.NUKE_GROUND_WAVE_SCALE.get();
         } catch (Throwable _t) {
            _waveScale = 1.0;
         }
         if (_waveScale <= 0.0) {
         ...
         long particleAmount = Math.round(Math.min(xRadius / 6.0 + 1.0, 45.0) * _waveScale);
         if (particleAmount <= 0L) {
            return;
         }
         if (time % 2.0 != 0.0) {
            return;
         ...
         for (int loop = 0; (long)loop < particleAmount; loop++) {
            ...
            if (y + 10.0 > (double)groundY) {
               net.mcreator.crustychunks.compat.WariumExplosions.explode(level, null, targetX, (double)(groundY + 11), targetZ, 8.0F, ExplosionInteraction.BLOCK);
            }

            if (time >= 5.0 && y + 5.0 > (double)groundY) {
               float blastPower = (float)(10.0 / Math.ceil(xRadius / 100.0 + 0.01) + 6.0);
               net.mcreator.crustychunks.compat.WariumExplosions.explode(level, null, targetX, Math.max((double)(groundY + 22), y + 7.0), targetZ, blastPower, ExplosionInteraction.BLOCK);
            }
         }
