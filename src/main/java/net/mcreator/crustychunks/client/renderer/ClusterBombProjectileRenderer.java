...
public class ClusterBombProjectileRenderer extends EntityRenderer<ClusterBombProjectileEntity> {
   private static final ResourceLocation texture = ResourceLocation.parse("crusty_chunks:textures/entities/clusterbomb.png");
   private final Modelheavybomb model;

   public ClusterBombProjectileRenderer(Context context) {
      super(context);
      this.model = new Modelheavybomb(context.bakeLayer(Modelheavybomb.LAYER_LOCATION));
   }

   public void render(
      ClusterBombProjectileEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn
   ) {
   ...
   public ResourceLocation getTextureLocation(ClusterBombProjectileEntity entity) {
      return texture;
   }
}
