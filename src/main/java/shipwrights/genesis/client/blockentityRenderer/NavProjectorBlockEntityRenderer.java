...
public class NavProjectorBlockEntityRenderer implements BlockEntityRenderer<NavProjectorBlockEntity> {
    ...
    public void render(NavProjectorBlockEntity blockEntity, float partialTick, PoseStack poseStack,
        ...
        for (Celestial body : registry) {
            renderCelestialProjection(poseStack, bufferSource, packedLight, packedOverlay, body, registry, currentPos, pos, scale_factor, blockRenderer, ticks, partialTick, body.type().equals(BuiltinCelestialTypes.STAR));
        }

        poseStack.popPose();
    }

    private static void renderCelestialProjection(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay, Celestial celestial, Registry<Celestial> registry, Vector3dc currentPlanetPos, BlockPos pos, int scale_factor, BlockRenderDispatcher blockRenderer, long ticks, float partialTick, boolean isStar) {
        Vector3d celestialPos = new Vector3d(celestial.getPosition(ticks, partialTick, registry));

        if (currentPlanetPos != null) {
            if (celestialPos.sub(new Vector3d(currentPlanetPos), new Vector3d()).length() > 120000) return;
        } else {
