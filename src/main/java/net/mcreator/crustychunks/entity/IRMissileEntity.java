...
public class IRMissileEntity extends AbstractArrow implements ItemSupplier {
   public static final ItemStack PROJECTILE_ITEM = new ItemStack((ItemLike)CrustyChunksModItems.ARMOR_PEELER_ROCKET.get());

   public IRMissileEntity(EntityType<? extends IRMissileEntity> type, Level world) {
      super(type, world);
   }
   ...
   public void tick() {
      super.tick();
      IRMissileFlightTickProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
   }

   public static IRMissileEntity shoot(Level world, LivingEntity entity, RandomSource source) {
      return shoot(world, entity, source, 1.0F, 5.0, 5);
   }

   public static IRMissileEntity shoot(Level world, LivingEntity entity, RandomSource source, float pullingPower) {
      return shoot(world, entity, source, pullingPower * 1.0F, 5.0, 5);
   }

   public static IRMissileEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
      IRMissileEntity entityarrow = new IRMissileEntity((EntityType<? extends IRMissileEntity>)CrustyChunksModEntities.IR_MISSILE.get(), entity, world);
   ...
   public static IRMissileEntity shoot(LivingEntity entity, LivingEntity target) {
      IRMissileEntity entityarrow = new IRMissileEntity(
         (EntityType<? extends IRMissileEntity>)CrustyChunksModEntities.IR_MISSILE.get(), entity, entity.level()
      );
