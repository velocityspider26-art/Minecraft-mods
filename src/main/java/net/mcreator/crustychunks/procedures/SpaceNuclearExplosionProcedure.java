...
public class SpaceNuclearExplosionProcedure {
   public static void execute(LevelAccessor world, double x, double y, double z, double power) {
      ...
      SpaceScorchProcedure.execute(world, x, y, z, 0.5 * explosionpower);
      SpacePlasmaCraterProcedure.execute(world, x, y, z, 0.5 * explosionpower);
      ...
      if (world instanceof Level _level && !_level.isClientSide()) {
         _level.explode(null, x, y, z, 60.0F, ExplosionInteraction.NONE);
      }

      if (60.0 < explosionpower) {
         ...
         Projectile _entityToSpawn = (new Object() {
               public Projectile getArrow(Level level, float damage, int knockback) {
                  AbstractArrow entityToSpawn = new SpaceThermalRadEntityEntity((EntityType<? extends SpaceThermalRadEntityEntity>)CrustyChunksModEntities.SPACE_THERMAL_RAD_ENTITY.get(), level) {
               @Override
               protected void doKnockback(LivingEntity livingEntity, DamageSource damageSource) {
                  if (knockback > 0) {
                     double _kbres = Math.max(0.0, 1.0 - livingEntity.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE));
