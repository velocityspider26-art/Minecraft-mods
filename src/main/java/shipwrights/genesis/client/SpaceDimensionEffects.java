...
public class SpaceDimensionEffects extends DimensionSpecialEffects {
    ...
    private void renderNearestStarGlow(ClientLevel level, float partialTick, PoseStack poseStack, Camera camera) {
        ...
        Vector3d camPosJoml = SpaceLevel.toCelestialSpace(level, camPos);
        ...
        float fade = (float) Mth.clamp(distance / 3000.0, 0.28, 1.0);
        if (fade <= 0.001f) {
            return;
        }
        ...
        float apparentHalfSize = (float) (glowDistance * (star.getActualSize() * 0.5)
                / Math.max(distance, 1.0));
        float outerRadius = Math.min(92.0f, apparentHalfSize * 1.78f);
        float innerAlpha = 0.22f * fade;
