...
public class LargeSmokeFireRenderer extends EntityRenderer<LargeSmokeFireEntity> {
   private static final ResourceLocation texture = ResourceLocation.parse("crusty_chunks:textures/entities/willyp.png");
   private final ModelHEATFS model;

   public LargeSmokeFireRenderer(Context context) {
      super(context);
      this.model = new ModelHEATFS(context.bakeLayer(ModelHEATFS.LAYER_LOCATION));
   }

   public void render(LargeSmokeFireEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
   ...
   public ResourceLocation getTextureLocation(LargeSmokeFireEntity entity) {
      return texture;
   }
}
