...
public class RevolverFireScriptProcedure {
   public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
      ...
      if (entity != null) {
         ...
         recoil = Mth.nextDouble(RandomSource.create(), 6.0, 8.0);
         if (entity.getData(CrustyChunksModVariables.PLAYER_VARIABLES)
            ...
            if ((entity instanceof LivingEntity _livEnt ? _livEnt.getOffhandItem() : ItemStack.EMPTY).getItem() == ItemStack.EMPTY.getItem()) {
               recoil = Mth.nextDouble(RandomSource.create(), 4.0, 6.0);
               Movementinnacuracy = 0.8;
            } else {
               recoil = Mth.nextDouble(RandomSource.create(), 6.0, 8.0);
            }
         }
         ...
         if ((entity instanceof LivingEntity _livEnt ? _livEnt.getOffhandItem() : ItemStack.EMPTY).getItem() == ItemStack.EMPTY.getItem()
            ...
            if (1.0 <= itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("Ammo")) {
               if (entity.getData(CrustyChunksModVariables.PLAYER_VARIABLES)
                  .AimDownSights) {
                  if (itemstack.getItem() instanceof RevolverAnimatedItem) {
                     CustomData.update(DataComponents.CUSTOM_DATA, itemstack, _tagupd -> _tagupd.putString("geckoAnim", "sightfire"));
                  }
               } else if (itemstack.getItem() instanceof RevolverAnimatedItem) {
         ...
         net.mcreator.crustychunks.compat.WariumSafety.report("RevolverFireScriptProcedure.execute", _wtSafe);
      }
   }
}
