...
public class IRMissileFlightTickProcedure {
   public static void execute(LevelAccessor world, double x, double y, double z, Entity immediatesourceentity) {
      ...
      if (immediatesourceentity != null) {
         ...
         double my = 0.0;
         double mz = 0.0;
         immediatesourceentity.getPersistentData().putDouble("MaxTime", 130.0);
         immediatesourceentity.getPersistentData().putDouble("Time", immediatesourceentity.getPersistentData().getDouble("Time") + 1.0);
         if (immediatesourceentity.getPersistentData().getDouble("Time") <= 130.0) {
         ...
         for (Entity entityiterator : world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(200.0), e -> true)
            ...
            if (entityiterator.getType().is(TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.parse("crusty_chunks:warm")))) {
               if (0.0 == entityiterator.getPersistentData().getDouble("Mx")
                  && 0.0 == entityiterator.getPersistentData().getDouble("My")
                  && 0.0 == entityiterator.getPersistentData().getDouble("Mz")) {
               ...
               distancetotarget = Math.sqrt(
                  Math.pow(Math.abs(entityiterator.getY() - y), 2.0)
                     + Math.pow(Math.abs(entityiterator.getX() - x), 2.0)
                     + Math.pow(Math.abs(entityiterator.getZ() - z), 2.0)
               );
               speed = Math.sqrt(
               ...
               if (Math.sqrt(
                           Math.pow(entityiterator.getY() - y, 2.0)
                              + Math.pow(entityiterator.getX() - x, 2.0)
                              + Math.pow(entityiterator.getZ() - z, 2.0)
                        )
                        - Math.sqrt(
                           Math.pow(entityiterator.getY() - (y + immediatesourceentity.getDeltaMovement().y()), 2.0)
                              + Math.pow(entityiterator.getX() - (x + immediatesourceentity.getDeltaMovement().x()), 2.0)
                              + Math.pow(entityiterator.getZ() - (z + immediatesourceentity.getDeltaMovement().z()), 2.0)
                        )
                     > 1.5
                  && immediatesourceentity.getPersistentData().getDouble("Time") >= 10.0) {
                  immediatesourceentity.setDeltaMovement(
                     new Vec3(
                        Math.min(
                           ...
                           Math.max(
                              (
                                    immediatesourceentity.getDeltaMovement().x() * 7.0
                                       + (entityiterator.getX() + mx * leadvariable - x) * speed / distancetotarget
                                 )
                                 / 8.0,
                              immediatesourceentity.getDeltaMovement().x() - Limiter
                           )
                        ...
                        Math.min(
                           ...
                           Math.max(
                              (
                                    immediatesourceentity.getDeltaMovement().y() * 7.0
                                       + (entityiterator.getY() + my * leadvariable - y) * speed / distancetotarget
                                 )
                                 / 8.0,
                              immediatesourceentity.getDeltaMovement().y() - Limiter
                           )
                        ...
                        Math.min(
                           ...
                           Math.max(
                              (
                                    immediatesourceentity.getDeltaMovement().z() * 7.0
                                       + (entityiterator.getZ() + mz * leadvariable - z) * speed / distancetotarget
                                 )
                                 / 8.0,
                              immediatesourceentity.getDeltaMovement().z() - Limiter
                           )
         ...
         if (speed > 12.0 && 10.0 < immediatesourceentity.getPersistentData().getDouble("Time")) {
            immediatesourceentity.setDeltaMovement(
               new Vec3(
         ...
         if (10.0 < immediatesourceentity.getPersistentData().getDouble("Time")) {
            Vec3 _center1 = new Vec3(x, y, z);

            for (Entity entityiteratorx : world.getEntitiesOfClass(Entity.class, new AABB(_center1, _center1).inflate(7.5), e -> true)
               .stream()
               ...
               if (entityiteratorx.getType().is(TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.parse("crusty_chunks:warm")))) {
                  detonate = true;
                  if (0.0 < (entityiteratorx instanceof Projectile _projEnt ? _projEnt.getDeltaMovement().length() : 0.0) && !entityiteratorx.level().isClientSide()) {
         ...
         for (Entity entityiteratorxx : world.getEntitiesOfClass(Entity.class, new AABB(_center2, _center2).inflate(1.0), e -> true)
            .stream()
         ...
         net.mcreator.crustychunks.compat.WariumSafety.report("IRMissileFlightTickProcedure.execute", _wtSafe);
      }
   }
}
