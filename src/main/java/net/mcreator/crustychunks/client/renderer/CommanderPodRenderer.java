...
public class CommanderPodRenderer extends EntityRenderer<CommanderPodEntity> {
   private static final ResourceLocation texture = ResourceLocation.parse("crusty_chunks:textures/entities/droppod.png");
   private final ModelFissionBomb model;

   public CommanderPodRenderer(Context context) {
      super(context);
      this.model = new ModelFissionBomb(context.bakeLayer(ModelFissionBomb.LAYER_LOCATION));
   }

   public void render(CommanderPodEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
   ...
   public ResourceLocation getTextureLocation(CommanderPodEntity entity) {
      return texture;
   }
}
