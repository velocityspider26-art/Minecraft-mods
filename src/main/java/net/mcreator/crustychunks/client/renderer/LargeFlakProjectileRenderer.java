...
public class LargeFlakProjectileRenderer extends EntityRenderer<LargeFlakProjectileEntity> {
   private static final ResourceLocation texture = ResourceLocation.parse("crusty_chunks:textures/entities/hefs.png");
   private final ModelHEATFS model;

   public LargeFlakProjectileRenderer(Context context) {
      super(context);
      this.model = new ModelHEATFS(context.bakeLayer(ModelHEATFS.LAYER_LOCATION));
   }

   public void render(
      LargeFlakProjectileEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn
   ) {
   ...
   public ResourceLocation getTextureLocation(LargeFlakProjectileEntity entity) {
      return texture;
   }
}
