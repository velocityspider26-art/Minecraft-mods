...
public class LargeBombProjectileRenderer extends EntityRenderer<LargeBombProjectileEntity> {
   ...
   public LargeBombProjectileRenderer(Context context) {
      super(context);
      this.model = new Modelheavybomb(context.bakeLayer(Modelheavybomb.LAYER_LOCATION));
   }

   public void render(
      LargeBombProjectileEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn
   ) {
   ...
   public ResourceLocation getTextureLocation(LargeBombProjectileEntity entity) {
      return texture;
   }
}
