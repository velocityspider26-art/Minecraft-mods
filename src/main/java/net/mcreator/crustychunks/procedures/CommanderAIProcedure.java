      ...
      if (entity != null) {
         ...
         if ((entity instanceof Mob _mobEntx ? _mobEntx.getTarget() : null) != null && (entity instanceof Mob _mobEnt ? _mobEnt.getTarget() : null).isAlive()) {
            ...
            if (Math.abs((entity instanceof Mob _mobEntxxxxxxx ? _mobEntxxxxxxx.getTarget() : null).getY() - y)
               ...
               entity.getPersistentData().putDouble("T2", 80.0);
               CrustyChunksMod.queueServerWork(
                  80,
                  () -> {
                     Vec3 _center = new Vec3(x, y, z);

                     for (Entity entityiterator : world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(200.0), ex -> true)
                        .stream()
                        .sorted(Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_center)))
                        .toList()) {
                        if (entity.isAlive()
                           && entityiterator.getType().is(TagKey.create(Registries.ENTITY_TYPE, ResourceLocation.parse("crusty_chunks:robot")))
                           && (entity instanceof Mob _mobEntxxxxxxxx ? _mobEntxxxxxxxx.getTarget() : null) != null
                           && entityiterator instanceof Mob) {
                           Mob _entity = (Mob)entityiterator;
                           Entity patt4449$temp = entity instanceof Mob _mobEntxxxxxxxxx ? _mobEntxxxxxxxxx.getTarget() : null;
                           if (patt4449$temp instanceof LivingEntity _ent) {
         ...
         if (entity.getPersistentData().getDouble("Cycle2") > 0.0) {
            entity.getPersistentData().putDouble("Cycle2", entity.getPersistentData().getDouble("Cycle2") - 1.0);
         } else {
            entity.getPersistentData().putDouble("Cycle2", 440.0);
            CrustyChunksMod.queueServerWork(
               1,
               () -> {
